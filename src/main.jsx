 ...
 * - Avoids crashes in SSR/prerender environments (where window/localStorage may be undefined)
...
const storage = {
  ...
  set(key, value) {
    try {
      ...
      // ignore write failures (private mode / quota / blocked)
    }
  },
};
...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    // Helpful during deploy debugging; safe to keep.
        ...
        <div className="min-h-screen bg-gray-50 text-gray-800 flex items-center justify-center p-8">
          <div className="max-w-lg w-full bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            ...
            <p className="text-sm text-gray-600 mb-4">
              The app hit an unexpected error. Refresh the page. If it keeps happening, check the console logs.
            </p>
...
function App() {
  const [activeTab, setActiveTab] = useState(() => storage.get("activeTab", "tasks"));
  const [darkMode, setDarkMode] = useState(() => storage.get("darkMode", false));
  const [currentTime, setCurrentTime] = useState(() => new Date());

  // Persisted state
  const [tasks, setTasks] = useState(() => storage.get("tasks", DEFAULT_TASKS));
  const [notes, setNotes] = useState(() => storage.get("notes", DEFAULT_NOTES));
  ...
  // Make Tailwind `dark:` variants actually work if your project uses class-based dark mode.
  useEffect(() => {
  ...
  const addNote = () => {
    ...
    setNotes((prev) => [
      ...prev,
      { id: Date.now(), title: title || "Untitled", content },
    ]);
  ...
  return (
    <div className={pageClass}>
      {/* Sidebar / Navigation */}
      <nav
        className={`fixed left-0 top-0 h-full w-20 flex flex-col items-center py-8 z-50 transition-colors ${
          ...
          <button
            ...
            className={`p-3 rounded-xl transition-all duration-200 group relative ${
              activeTab === "tasks"
                ...
                : "text-gray-400 hover:text-indigo-500 hover:bg-gray-100 dark:hover:bg-slate-700"
            }`}
        ...
        <div className="mt-auto flex flex-col gap-4">
          <button
            ...
            className={`p-3 rounded-xl transition-all ${
              darkMode ? "text-yellow-400 hover:bg-slate-700" : "text-slate-400 hover:bg-gray-100 hover:text-slate-600"
            }`}
          ...
          {activeTab === "notes" && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {notes.map((note) => (
                <div
                  key={note.id}
                  className={`relative group p-6 h-64 rounded-2xl border shadow-sm flex flex-col ${
                ...
                <div className="flex justify-end gap-3">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium ${
                      darkMode ? "hover:bg-slate-800 text-slate-300" : "hover:bg-gray-100 text-gray-600"
                    }`}
                  >
                    Cancel
                  </button>
